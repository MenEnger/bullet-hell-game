...
- **M**: 弾を発射（最も近い敵に向かう追尾弾）
- **ポーズボタン**: ゲームの一時停止/再開
- **ミュートボタン**: BGMと効果音のオン/オフ（設定はlocalStorageに保存）
...
### BGMと効果音
- **BGM**: Web Audio APIによる周波数変化するループトーン（C-E-G-Cの音階パターン）
- **効果音**: ZzFXライブラリによる6種類の効果音
  - プレイヤー発射音
  - 敵撃破音（着弾）
  - プレイヤー被弾音
  - ゲームオーバー音
  - ゲームクリア音
- 音声は事前生成してキャッシュされ、再生時のパフォーマンスへの影響を最小化
- ミュート設定はlocalStorageに保存され、次回起動時も維持
...
- **ビルドツール**: Vite
- **レンダリング**: Canvas API
- **オーディオ**: ZzFX (効果音) + Web Audio API (BGM)
- **スタイル**: CSS
...
- `playHit()`: 着弾音を再生
- `playDamage()`: 被弾音を再生
- `playGameOver()`: ゲームオーバー音を再生
- `playGameClear()`: ゲームクリア音を再生
- `toggleMute()`: ミュート状態をトグル
- 音声は事前生成してキャッシュし、再生時のパフォーマンスを最適化
